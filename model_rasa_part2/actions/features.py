...
class bot_memory:
    ...
    def slotsetter(self,query,data):
        if ("SELECT showname" in query or "SELECT prodtitle" in query) and len(data)==1 and data[0][0] != '':
            ...
            Bot.city = None
            Bot.code = None
        elif ("SELECT city" in query or "SELECT market_type_code" in query) and len(data)==1 and data[0][0] != '':
...
def productionTable(full_message,broadway_show,cityCode,city): 
    ...
    try:
        ...
        prompt = f"you are an exception handler of chatbot of https://www.broadwayworld.com/, chatbot got an error while fetching data using query: {query}, conversation was going on {broadway_show} in {city}, users current questiion is :{full_message}\n Now if query has something None then ask user for that like city or broadway name, if current question is not from previous context then handle it with grace like a gentle man and say sorry for not having it's answer"
...
def castTable(prompt,broadway_show):
    ...
    cast.add_example(Example("can you tell me about the cast of The Lady Comes Across",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%The Lady Comes Across%' AND market_type_code = 'BR') AND role is not NULL AND role <>'' AND credited_name IS NOT NULL AND credited_name <> '' LIMIT 45;"))
    cast.add_example(Example("Who played the role of Jean Valjean in the recent revival of Les Misérables?",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%Les Misérables%' AND market_type_code = 'BR') AND role LIKE '%Jean%' AND credited_name IS NOT NULL AND credited_name <> '' LIMIT 45;"))
    cast.add_example(Example("who has done lead role in Hamilton broadway",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%Hamilton%' AND market_type_code = 'BR') AND role is not NULL AND role <>'' AND credited_name IS NOT NULL AND credited_name <> '' LIMIT 45;"))
    ...
    cast.add_example(Example("Who played Character Tony",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%{broadway_show}%' AND market_type_code = 'BR') AND role LIKE '%Tony%' AND role is not NULL AND role <>'' LIMIT 45;"))
    cast.add_example(Example("who played Betsy Hamilton in the show",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%Hamilton%' AND market_type_code = 'BR') AND role LIKE '%Betsy%' AND role is not NULL AND role <>'' LIMIT 45;"))
    cast.add_example(Example("which actor played Usherette in the broadway show",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%{broadway_show}%' AND market_type_code = 'BR') AND role LIKE '%Usherette%' AND role is not NULL AND role <>'' LIMIT 45;"))
    cast.add_example(Example("who played the role of Aaron",f"SELECT role,credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%{broadway_show}%' AND market_type_code = 'BR') AND role LIKE '%Aaron%' AND role is not NULL AND role <>'' AND credited_name IS NOT NULL AND credited_name <> '' LIMIT 45;"))
    cast.add_example(Example("just name all the crew members not their roles",f"SELECT credited_name FROM cast WHERE productions_id IN (SELECT id FROM productions WHERE prodtitle LIKE '%{broadway_show}%' AND market_type_code = 'BR') AND credited_name IS NOT NULL AND credited_name <> '' LIMIT 45;"))
